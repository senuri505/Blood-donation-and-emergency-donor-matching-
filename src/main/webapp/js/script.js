...
function handleMockPost(url, data) {
    ...
    if (servlet === 'RegisterServlet') {
        ...
        if (role === 'donor') {
            const donors = JSON.parse(localStorage.getItem('hemolink_donors') || '[]');
            const maxDonorId = donors.reduce((max, d) => Math.max(max, d.id || 0), 0);
            donors.push({
                id: maxDonorId + 1,
                userId: newUserId,
                name: data.name,
            ...
            const hospitals = JSON.parse(localStorage.getItem('hemolink_hospitals') || '[]');
            const maxHospId = hospitals.reduce((max, h) => Math.max(max, h.id || 0), 0);
            hospitals.push({
                id: maxHospId + 1,
                userId: newUserId,
                name: data.name,
    ...
    if (servlet === 'DonorServlet') {
        ...
        if (action === 'approve' || action === 'reject') {
            ...
            return { success: false, message: 'Donor not found with id ' + data.id };
        }
        if (action === 'delete') {
            // Find by donor id or userId
            const targetId = parseInt(data.id);
            const targetUserId = parseInt(data.userId);
            const before = donors.length;
            donors = donors.filter(x => x.id != targetId && x.userId != targetUserId);
            localStorage.setItem('hemolink_donors', JSON.stringify(donors));
            // Also remove from users list
            let users = JSON.parse(localStorage.getItem('hemolink_users') || '[]');
            users = users.filter(u => u.id != targetUserId);
            localStorage.setItem('hemolink_users', JSON.stringify(users));
            const deleted = before > donors.length;
            return { success: true, message: deleted ? 'Donor deleted successfully.' : 'User record removed.' };
        }
    ...
    if (servlet === 'HospitalServlet') {
        let hospitals = JSON.parse(localStorage.getItem('hemolink_hospitals') || '[]');
        if (action === 'approve' || action === 'reject') {
            const h = hospitals.find(x => x.id == data.id);
            if (h) {
                h.approvalStatus = action === 'approve' ? 'approved' : 'rejected';
                localStorage.setItem('hemolink_hospitals', JSON.stringify(hospitals));
                return { success: true, message: 'Hospital status updated' };
            }
            return { success: false, message: 'Hospital not found with id ' + data.id };
        }
        if (action === 'delete') {
            const targetId = parseInt(data.id);
            const targetUserId = parseInt(data.userId);
            const before = hospitals.length;
            hospitals = hospitals.filter(x => x.id != targetId && x.userId != targetUserId);
            localStorage.setItem('hemolink_hospitals', JSON.stringify(hospitals));
            // Also remove from users list
            let users = JSON.parse(localStorage.getItem('hemolink_users') || '[]');
            users = users.filter(u => u.id != targetUserId);
            localStorage.setItem('hemolink_users', JSON.stringify(users));
            const deleted = before > hospitals.length;
            return { success: true, message: deleted ? 'Hospital deleted successfully.' : 'User record removed.' };
        }
    }
